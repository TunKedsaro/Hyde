...
def build_user_context(student_row: Dict[str, Any]) -> UserContextArtifacts:
    ...
    edu_level = student_row.get("education_level") or "unknown"
    edu_major = student_row.get("education_major") or ""

    # ------------------------------------------------------------------
    # Optional profile fields
    # ------------------------------------------------------------------
    target_roles_raw = (student_row.get("target_roles") or "").strip()
    skills_raw = (student_row.get("skills") or "").strip()
    ...
    if preferred_language == "th":
        user_context_text = (
            f"นักศึกษา: {edu_major} ({current_status})\n"
        ...
        user_context_text = (
            f"Student major: {edu_major} ({current_status}).\n"
